...
export default function GamePlay() {
  ...
  function reset(){ setScore(0); setTimeLeft(15); setWon(false) }

  return (
    <div className="bg-white/70 rounded-xl p-4 shadow-md">
      <h3 className="font-semibold text-center">Pop 3 happy faces — super easy! 🎈</h3>
      <p className="text-xs text-center mt-1">Tap the faces. You have {timeLeft}s</p>

      <div className="relative h-40 mt-4 overflow-hidden">
        {[0,1,2,3,4].map(i=> (
          <motion.img key={i} src="/images/meme1.jpg" alt="face" onClick={pop}
      ...
      <div className="mt-3 flex gap-2 justify-center">
        <button onClick={reset} className="px-3 py-1 rounded bg-gray-200">Play again</button>
      </div>
    </div>
  )
}
